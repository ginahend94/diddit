...
export const taskDetails = task => {
    const showDetailModal = (() => {
        const modalInner = () => {
            ...
            taskDetails.append(buttons);
            buttons.style = 'display:flex; gap:5px';
            ...
            deleteButton.prepend(getIcon('trash-can-outline'));
            deleteButton.append('Delete task');
            deleteButton.classList.add('cancel');
            deleteButton.style.width = 'fit-content';
            deleteButton.addEventListener('click', e => {
                const modalInner = () => {
                    const div = document.createElement('div');
                    div.innerHTML = `Are you sure you want to delete "${task.name}"? <br /><strong>This cannot be undone.</strong>`
                    return div;
                }
